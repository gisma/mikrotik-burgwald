---
title: "Mikrotek Setup in der TTN"
...
Hier folgt eine basale Anleitung für den Betrieb eines MikroTik-LoRaWAN-Gateways (RBwAPR-2nD) über WLAN mit TTN (EU1) sowie der Einbindung der Sensoren **Dragino PS-LB**, **Dragino DDS-75LB** und **Seeed SenseCAP S2120**. Die **konkreten Decoder** sind nicht enthalten aber auf dem Gateway bereits aktiv. Zusätzlich werden praxistaugliche Wege zum **Dauerbetrieb** (Storage, MQTT→CSV/DB, InfluxDB/Grafana) und **No-Code-Speicherung** (Webhooks) skizziert. Beispiele und Snippets sind so gehalten, dass sie direkt übernommen werden können.

# TTN Community Cloud

The Things Network (TTN) ist ein öffentliches, community-betriebenes LoRaWAN-Netz: Gateways sammeln Funksignale aller angemeldeten Sensoren und leiten sie über einen zentralen Netzwerkserver an eigene Anwendungen weiter. Daher muss kein eigener LoRaWAN-Server aufgesetzt werden – Geräte registrieren, Schlüssel hinterlegen, und die Daten erscheinen (best-effort, kostenlos) und sind in der TTN-Console und per MQTT/Webhooks verfügbar.

## Warum TTN Community Cloud ?

-   **LoRaWAN Netzwerkbetrieb**: Network-/Join-/Application-Server, Device-Registry, Session-Keys, RX1/RX2-Downlink-Scheduling.

-   **Protokoll & Funk-Optimierung**: ADR, MAC/PHY-Updates, Region/Frequency-Plans, Duty-Cycle-Handling.

-   **Skalierung & Verfügbarkeit**: Clusterbetrieb, Load-Balancing, Wartung/Upgrades im Hintergrund.

-   **Sicherheit**: TLS, Key-Handling, Rechte/Rollen, Abuse-/Rate-Limit-Schutz.

-   **Integrationen**: MQTT, Webhooks, HTTP/gRPC-APIs, Payload-Formatter, Storage-Forwarding.

-   **Observability**: Basis-Monitoring/Logs (Live Data, Events), Fehlerpfade sichtbar ohne eigenes Monitoring.

-   **Roaming/Weitverkehr**: Packet Broker-Anbindung (Gateways/Netze übergreifend), ohne eigene Peering-Pflege.

-   **Warum TTN Community für \~100 Sensoren:**

    -   **0 € & wartungsfrei**, sofort startklar, bewährte Defaults.
    -   **Skaliert** bei Intervallen 1–60 min problemlos.
    -   **Ops-Aufwand ≈ 0** im Vergleich zu self-hosted.

-   **Alternativen (kurz):**

    -   **Loriot / Actility (Cloud)**: gemanagt, **kostenpflichtig**, SLA.
    -   **AWS IoT Core for LoRaWAN**: pay-as-you-go, tiefe AWS-Integration.
    -   **ChirpStack (self-hosted)**: Lizenz 0 €, aber voller Betriebs-/Sicherheits-/HA-Aufwand.

# Setup Mikrotek Fast Track

## Script hochladen

-   In **WinBox → Files**: Setup-Datei [`burgwald_all.rsc`](assets/burgwald_all.rsc) einfach per Drag&Drop auf das Gerät laden. [**WinBox Download (MikroTik)**](https://mikrotik.com/download)
...
### Im einzelnen: MikroTik-Konnektivität, MAC-Zugriff und Hotspot-Einstellungen
...
# Die Langversion falls es klemmt
...
-   **WinBox Download** – Offizielle Management-GUI (Windows; Linux via Wine). <https://mikrotik.com/download>
-   **RouterOS LoRa Dokumentation** – LoRaWAN-Setup, Forwarding, Frequency Plans. <https://help.mikrotik.com/docs/display/ROS/LoRa>

### Dragino (PS-LB/LS & Co.)

-   **Dragino Wiki (Übersicht)** – Zentrale Manuals, AT/Downlink-Befehle, FAQs. <https://wiki.dragino.com/xwiki/bin/view/Main/>
-   **Payload-Decoder (GitHub)** – Offizielle Decoder für End-Nodes (z. B. PS-LB). <https://github.com/dragino/dragino-end-node-decoder/tree/main>

### Seeed SenseCAP S2120

-   **Getting Started** – Schritt-für-Schritt Inbetriebnahme. <https://wiki.seeedstudio.com/Getting_Started_with_SenseCAP_S2120_8-in-1_LoRaWAN_Weather_Sensor/>
-   **User Guide (PDF)** – Vollständiges Handbuch. <https://files.seeedstudio.com/products/SenseCAP/101990961_SenseCAP%20S2120/SenseCAP%20S2120%20LoRaWAN%208-in-1%20Weather%20Station%20User%20Guide.pdf>
-   **TTN-Decoder (GitHub)** – JavaScript-Decoder für TTN/ChirpStack. <https://github.com/Seeed-Solution/TTN-Payload-Decoder/blob/master/SenseCAP_S2120_Weather_Station_Decoder.js>

### The Things Network (TTN)

-   **Website** – Überblick, Community, News. <https://www.thethingsnetwork.org/>
-   **Console (Cluster-Auswahl)** – Apps/Geräte verwalten (eu1 etc.). <https://console.cloud.thethings.network/>
-   **Console EU1 (Direktlink)** – Login auf dem EU1-Cluster. <https://eu1.cloud.thethings.network/>
-   **Docs / Learn** – Handbücher, Beispiele, API-Referenzen. <https://www.thethingsnetwork.org/docs/>
