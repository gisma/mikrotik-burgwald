...
Diese können in `.env`, in der Shell oder im GitHub Actions Workflow (`env:`) gesetzt werden.
...
* `TTN_AFTER_DAYS` (`"2"`)
  Gleitendes Fenster: *jetzt − N Tage*.
...
* `assets/`
  ...
  * `devices_used.txt` — **lesbarer** Health-Report
  * `devices_used.csv` — **maschinenlesbarer** Health-Report
...
* **HTTP 400 invalid token**
  Neuer Key nötig oder Scopes fehlen.
* **HTTP 400 mit `after`**
...
* **Keine Daten**
  TTN Console prüfen: Storage aktiviert? Device aktiv? Zeitfenster korrekt?
* **429 Rate limit**
  `DELAY_BETWEEN_DEVICES` auf 0.6–1.0 erhöhen.
* **Viele STALE-Geräte**
