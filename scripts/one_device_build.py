...
# Sandbox: nur 24h Storage → 1 Tag rückwärts
...
def device_pull(dev):
    """Pullt NDJSON nur für dieses Device, merged in Parquet und liefert DataFrame zurück (letzte 24h)."""
...
def unpack_payload(d):
    """Passe hier deine Decoder-Keys an (Beispiele)."""
...
for dev in DEVS:
    ...
    all_last_seen.append((dev, last_ts))

    # Kleine pro-Device Plots (nur, wenn Spalten existieren)
    labels = {
        "pm25": "PM2.5 (µg/m³)",
        "co2": "CO₂ (ppm)",
